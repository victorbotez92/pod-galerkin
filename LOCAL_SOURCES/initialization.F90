...
#include "petsc/finclude/petsc.h"
   ...
   USE user_data
   IMPLICIT NONE
   PUBLIC ::  initial, run_pod_galerkin
!  PUBLIC :: run_compute_DR, initial, save_run, run_SFEMaNS
   PUBLIC :: prodmat_maxwell_int_by_parts
   ! MC-CN 06/03/2024
!  PUBLIC :: suites_to_bins, suites_to_bins_from_file, bins_to_paraview_from_file
!  PUBLIC :: old_nsample_to_new_phys, old_nsample_to_new_fourier
   PRIVATE
   ...
   TYPE(petsc_csr_LA) :: LA_H_3
   TYPE(petsc_csr_LA) :: LA_H_1
...
!!$        un_out, un_m1_out, pn_out, Hn_out, Bn_out, phin_out, v_to_Max_out, &
...
!!$        tempn_out, concn_out, level_set_out, density_out, der_un_out, visc_LES_out)
! CN 15/03/2024
  SUBROUTINE initial(vv_mesh_out, pp_mesh_out, H_mesh_out, phi_mesh_out, temp_mesh_out, &
       conc_mesh_out,interface_H_phi_out, interface_H_mu_out, list_mode_out, &
...
#include "petsc/finclude/petsc.h"
    USE petsc
      ...
      REAL(KIND = 8), POINTER, DIMENSION(:, :, :) :: un_m1_out
   ...
   !---------------------------------------------------------------------------
   SUBROUTINE run_pod_galerkin()
      USE restart
      USE input_data
      USE pod_galerkin
      USE pod_galerkin_mag
      USE pdf
      USE fft_DR
      ...
      INTEGER :: i, m_i, m_j, m_k, m, m_max_c, index, vv_m_max_pad, mode, lblank, l, code, rank, vv_rang_F,vv_rang_S, H_rang_F, H_rang_S 
      INTEGER :: vv_nb_procs_F, H_nb_procs_F, vv_bloc_size_gauss, H_bloc_size_gauss
      REAL(KIND = 8), DIMENSION(2, vv_mesh%gauss%l_G * vv_mesh%dom_me) :: vv_rr_gauss
      ...
      REAL(KIND = 8), DIMENSION(user%ending_it-user%starting_it+1,user%ending_it-user%starting_it+1) :: L_ij
      REAL(KIND = 8), DIMENSION(user%ending_it-user%starting_it+1,user%ending_it-user%starting_it+1,user%ending_it-user%starting_it+1) :: Q_ijk
      ...
      CALL MPI_COMM_SIZE(comm_one_d_ns(2), vv_nb_procs_F, code)
      CALL MPI_COMM_SIZE(comm_one_d(2), H_nb_procs_F, code)
      CALL MPI_COMM_RANK(comm_one_d_ns(2), vv_rang_F, code)
      CALL MPI_COMM_RANK(comm_one_d(2), H_rang_F, code)
      CALL MPI_COMM_RANK(comm_one_d_ns(1), vv_rang_S, code)
      CALL MPI_COMM_RANK(comm_one_d(1), H_rang_S, code)

      
      IF (user%mag_galerkin .AND. user%if_B) THEN
         CALL compute_K_ijk_and_N_ij(comm_one_d_ns, comm_one_d, vv_1_LA, vv_3_LA, LA_H_1, LA_H_3, vvrtz_per, vvz_per, &
         pp_mesh, vv_mesh, H_mesh, jj_v_to_H, list_mode)
      END IF

      IF (user%vel_galerkin) THEN
         CALL compute_L_ij_and_Q_ijk(comm_one_d_ns, comm_one_d, vv_1_LA, vv_3_LA, LA_H_1, LA_H_3, vvrtz_per, vvz_per, &
         pp_mesh, vv_mesh, H_mesh, jj_v_to_H, list_mode)
      END IF
      
   END SUBROUTINE run_pod_galerkin
    ...
    USE petsc
      ...
      IF (if_momentum .OR. inputs%type_pb=='mxx') THEN
         ...
         IF (pp_mesh%me/=0) THEN
            ...
            IF (.NOT.(user%if_recompute_DR)) THEN
      ...
      IF (if_induction) THEN
         ...
         CALL st_aij_csr_glob_block(comm_one_d(1), 3, H_mesh_glob, H_mesh, LA_H_3)
         CALL st_aij_csr_glob_block(comm_one_d(1), 1, H_mesh_glob, H_mesh, LA_H_1)
      ...
      IF (.NOT.(inputs%if_just_processing.AND.user%if_DR_postpro) .AND. &
           .NOT.(user%if_recompute_DR)) THEN
         ...
         IF (if_momentum .OR. inputs%type_pb=='mxx') THEN
            ...
            END IF
         END IF
      ELSEIF (user%if_recompute_DR) THEN
         IF (if_momentum .OR. inputs%type_pb=='mxx') THEN
            ALLOCATE(un(vv_mesh%np, 6, m_max_c))
         END IF
      END IF
         ...
         IF (if_momentum .OR. inputs%type_pb=='mxx') THEN
            IF (vv_mesh%me/=0) THEN
               IF (inputs%irestart_u) THEN
                  ...
                  ELSE
                     IF (.NOT. if_mass) THEN
                        IF (inputs%LES .AND. inputs%irestart_LES) THEN
                           CALL read_restart_ns(comm_one_d_ns, time_u, list_mode, un, un_m1, pn, pn_m1, &
                                incpn, incpn_m1, inputs%file_name, opt_LES = visc_LES)
                        ELSE
                           CALL read_restart_ns(comm_one_d_ns, time_u, list_mode, un, un_m1, pn, pn_m1, &
      ...
      !===Initialize Maxwell==========================================================
      time_h = 0.d0
      IF ((if_induction).AND.(.NOT.(user%if_recompute_DR))) THEN
